/**
 * URL Controller
 * Handles HTTP requests and responses for URL shortening operations
 */
...
class UrlController {
  ...
  async createShortUrl(req, res) {
    try {
      ...
      logger.error('Controller error in createShortUrl', { error: error.message });
      res.status(500).json({
        success: false,
  ...
  async redirectToUrl(req, res) {
    try {
      ...
      logger.error('Controller error in redirectToUrl', { error: error.message });
      res.status(500).json({
        success: false,
        error: 'Internal server error'
      });
      ...
      endpoints: {
        'POST /shorturls': {
          ...
          requestBody: {
            ...
            validity: 'number (optional) - Expiration time in minutes (default: 30)',
        ...
        'GET /shorturls/:shortcode': {
          ...
          response: {
            ...
            data: {
              ...
              expiry: 'string - ISO 8601 timestamp',
              clickEvents: 'array - Array of click events with geolocation data'
            }
          }
        },
        'GET /:shortcode': {
